...
laberint::laberint(std::istream &is) throw(error)
{
    ...
    while (getline(is, linia))
    {
        ...
        //std::cout << "linia: " << linia << "contLinia: " << contLinia << std::endl;
        if (primLinia) //Obtenim el tamany de les files i de les columnes
        {
            ...
            //std::cout<<"fila: "<<_nFil<<"cols: "<<_nCol<<std::endl;
            _lab = new cambra *[_nFil];
        ...
        else //Obtenim les linies del laberint
        {
            ...
            for (int j = 0; j < _nCol; j++)
            {
                ...
                int jlin = j * 2 + 1; // obtenim la pos de la porta

                // La funcio obre porta ja s'encarrega de restar una unitat a la coordenada
                p.first = contLinia;
                p.second = j + 1;
                if (linia[jlin] == ' ')
                {
                    ...
                    //std::cout<<"linies: "<<_nFil<<" liniaActual: "<<p.first<<std::endl;
                    //std::cout<<"columnes: "<<_nCol<<" colActual: "<<p.second<<std::endl;
...
{
    if (pos.first >= _nFil or pos.second >= _nCol)
    { // Al començar des de 0 li hem de restar 1, per això es >=.
...
void laberint::obre_porta(paret p, const posicio &pos) throw(error)
{
    ...
    { // Al començar des de 0 li hem de restar 1, per això es >=.
        ...
        throw error(PortaExterior); // comprovar si la paret està al perímetre del laberint.
...
void laberint::print(std::ostream &os) const throw()
{
    ...
    for (nat i = 0; i < _nFil; i++)
    {
        ...
        { //Pintem parets verticals menys la última del sud
...
bool laberint::portaExterior(paret p, posicio pos)
{
    ...
    if (pos.first - 1 == 0 and p == paret::NORD)
        return true;
    else if (pos.first - 1 == _nFil and p == paret::SUD)
        return true;
    else if (pos.second - 1 == 0 and p == paret::OEST)
        return true;
    else if (pos.second - 1 == _nCol and p == paret::EST)
        return true;
